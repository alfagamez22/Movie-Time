        ...
        return (
          <button
            ...
            className={`group flex w-full touch-manipulation select-none gap-3 border-b border-white/5 p-3 text-left transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-white ${
              isActive
                ? 'bg-white/10'
                : isUpcoming
                  ? 'cursor-not-allowed bg-white/2 opacity-55'
                  : isWatched
          ...
          >
            ...
            <div className="min-w-0 flex-1 py-0.5">
              ...
              <p className={`mt-0.5 text-[11px] font-medium uppercase tracking-[0.12em] ${isUpcoming ? 'text-amber-300' : 'text-zinc-400'}`}>
                Episode {episodeNumber}
                {isUpcoming ? ` · ${upcomingLabel ?? 'Upcoming'}` : isWatched && !isActive ? ' · Watched' : ''}
              </p>
...
function SidebarControls({
  ...
  return (
    <div className="flex min-h-0 w-full flex-1 flex-col overflow-hidden border-t border-white/5 bg-[#101014] landscape:h-full landscape:w-[clamp(18rem,30vw,23rem)] landscape:flex-none landscape:border-l landscape:border-t-0">
